...
Вызовите `memory_read_graph` (правила и шаблоны — в [MEMORY.md](MEMORY.md)). Найдите релевантные `MetricsLesson` по типу продукта и уровню. Применяйте их при выборе метрик.
...
При отсутствии данных у пользователя — найдите бенчмарки LTV/CAC и конверсий по индустрии через `tavily_tavily-search` (MCP). Источники зафиксируйте в Excel (отдельная колонка / лист «Источники»).
...
Также создайте/обновите `PROJECTS/<Имя>/PROJECT.md` по шаблону `PROJECTS/_template/PROJECT.md` (контекст, ключевые решения, применённые уроки из памяти, риски, артефакты).

При получении фидбека от пользователя — сохраните урок в память (правила в [MEMORY.md](MEMORY.md)).
...
- [MEMORY.md](MEMORY.md) — работа с Knowledge Graph Memory (шаг 0).
